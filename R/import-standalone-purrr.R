...
map2 <- function(.x, .y, .f, ...) {
    .f <- rlang::as_function(.f, env = rlang::global_env())
    out <- mapply(FUN = .f, .x, .y, MoreArgs = list(...), SIMPLIFY = FALSE)
...
map2_lgl <- function(.x, .y, .f, ...) {
    as.vector(x = map2(.x, .y, .f, ...), mode = "logical")
}
map2_int <- function(.x, .y, .f, ...) {
    as.vector(x = map2(.x, .y, .f, ...), mode = "integer")
}
map2_dbl <- function(.x, .y, .f, ...) {
    as.vector(x = map2(.x, .y, .f, ...), mode = "double")
}
map2_chr <- function(.x, .y, .f, ...) {
    as.vector(x = map2(.x, .y, .f, ...), mode = "character")
}
imap <- function(.x, .f, ...) {
...
pmap <- function(.l, .f, ...) {
    .f <- as.function(x = .f)
    args <- .rlang_purrr_args_recycle(args = .l)
    do.call(what = "mapply", args = c(FUN = list(quote(.f)), args, MoreArgs = quote(list(...)), SIMPLIFY = FALSE, USE.NAMES = FALSE))
}
.rlang_purrr_args_recycle <- function(args) {
    lengths <- map_int(.x = args, .f = length)
    n <- max(lengths)
    stopifnot(all(lengths == 1L | lengths == n))
    to_recycle <- lengths == 1L
    args[to_recycle] <- map(.x = args[to_recycle], .f = function(x) rep.int(x = x, times = n))
    args
}
...
map_if <- function(.x, .p, .f, ...) {
    matches <- .rlang_purrr_probe(.x = .x, .p = .p)
...
reduce <- function(.x, .f, ..., .init) {
    ...
    Reduce(f = f, x = .x, init = .init)
}
reduce_right <- function(.x, .f, ..., .init) {
    f <- function(x, y) .f(y, x, ...)
    Reduce(f = f, x = .x, init = .init, right = TRUE)
}
accumulate <- function(.x, .f, ..., .init) {
    f <- function(x, y) .f(x, y, ...)
    Reduce(f = f, x = .x, init = .init, accumulate = TRUE)
}
