...
Si bien existen muchos proyectos de código abierto para el desarrollo de paquetes de R, muchas tareas tediosas y repetitivas en el proceso de desarrollo pueden simplificarse mediante la funcionalización. El objetivo de rpkgkit es proporcionar funciones útiles para el desarrollo de paquetes de R.
...
Todas las funciones pueden detectar el contexto del archivo activo en RStudio y Positron, por lo que generalmente se puede omitir la ruta del archivo.

### Scripts Independientes Disponibles

Usa `usethis::use_standalone("WangLabCSU/rpkgkit", "<name>")` para importar:

- args_to_func.R: empareja argumentos con llamadas a funciones:
...
# Emparejamiento estricto (por defecto): devuelve solo f1
...
filter_args_for_func(args, foo) # Conserva los argumentos para foo
# $x
# [1] 3
...
- caller_cli.R: muestra desde dónde se llama a la función cli
...
- colorful_cli.R: invocación de colores más sencilla en una sola función cli
...
color_cli2 <- create_colorful_cli_env(cli_theme = generate_color_theme()) # más colores pero más lento
color_cli2$cli_alert_success(
  "{.violetred3 R}{.orange a}{.yellow i}{.green n}{.cyan b}{.blue o}{.purple w}"
)
```

- match_arg.R: coincidencia parcial de argumentos con llamadas a funciones, como `match.arg`, `rlang::arg_match`

- ts_cli.R: función cli con marca de tiempo
...
### Gestión de Archivos Independientes

- `inquire_standalone()` — Lista los archivos independientes disponibles en el directorio `R/` de un repositorio de GitHub

- `browse_standalone()` — Consulta todos los archivos independientes disponibles en repositorios de GitHub
...
- `create_standalone()` — Crea archivos de utilidad independientes en tu paquete
...
- `update_time_in_standalone()` — Actualiza el campo `last-updated` en archivos independientes
...
- `add_changelog_in_standalone()` — Agrega entradas de registro de cambios a los archivos independientes
...
- `news_md_add_entry()` — Agrega nuevas entradas a NEWS.md siguiendo las directrices de CRAN
...
- `news_md_check()` — Valida el formato de NEWS.md para el cumplimiento con CRAN
...
- `news_md_show()` — Muestra el contenido de NEWS.md de un paquete en la consola con color

### Transformación de Funciones R

- `make_func_call_explicit()` — Hace explícitas las llamadas a funciones añadiendo prefijos de paquete
- `package_func_call_explicit()` — Hace explícitas las llamadas a funciones añadiendo prefijos de paquete en un paquete

Este es un fragmento de código de [dplyr](https://github.com/tidyverse/dplyr):
...
- `detect_lost_glue_brace()` — Encuentra todas las llamadas a `glue` a las que les falta una llave de cierre en un archivo. Soporta expresiones `glue` y `cli`.
- `package_lost_glue_brace()` — Encuentra todas las llamadas a `glue` a las que les falta una llave de cierre en un paquete. Soporta expresiones `glue` y `cli`.
...
- `make_func_arg_explicit()` — Hace que los argumentos de las funciones se pasen con nombres de parámetros explícitos
- `package_func_arg_explicit()` — Hace que los argumentos de las funciones se pasen con nombres de parámetros explícitos en un paquete
...
- `rename_func()` — Renombra funciones en un archivo con un estilo específico
...
- `detect_print_and_cat()` — Detecta llamadas a `print()` y `cat()` en un archivo
- `package_print_and_cat()` — Detecta llamadas a `print()` y `cat()` en un paquete

`print()` y `cat()` no están permitidos según la política de CRAN, por lo que es necesario corregirlos con `message()`
...
- `convert_func_syntax()` — Convierte la sintaxis de funciones entre `function()` y `\()`
...
``` r
# * Ej., úsalo en el entorno de desarrollo de rpkgkit
use_zzz()
...
- `check_pkgdown_reference()` — Verifica si todas las funciones exportadas están referenciadas en `_pkgdown.yml`
...
- `use_vendor()` — Referencia un paquete R con licencia permisiva desde GitHub para incluirlo en tu propio paquete R. Facilita la importación de paquetes R de GitHub bajo la política de CRAN.

La licencia, los derechos de autor y la declaración se generan automáticamente en `DESCRIPTION`, `R/vendor-*.R` e `inst/vendor/`.
...
- `use_multilanguage_readme()` — Crea una plantilla README.md multilingüe para tu paquete R.
- `badge_translated_by_ai()` — Crea una insignia de "traducido por IA".
...
- `Add_global_gitignore()` — Agrega un archivo `.gitignore` global a tu paquete R.

## Agradecimientos

Queremos agradecer a las siguientes personas y proyectos:

- Los autores del paquete [pedant](https://github.com/wurli/pedant) — **Jacob Scott**, **Christopher T. Kenny**, y **Sebastian Lammers** — cuyo código está incluido (bajo licencia MIT) en `R/vendor-pedant.R`.
- Los autores del paquete [pkgdev](https://github.com/dieghernan/pkgdev) — **Diego Hernangómez** — cuyo código está incluido (bajo licencia MIT) en `R/vendor-pkgdev.R`.
- Todos los colaboradores y usuarios que han reportado problemas, sugerido funcionalidades o ayudado a mejorar el paquete.
