...
#' }
news_md_add_entry <- function(entry,
                               version = NULL,
                               category = "NEW FEATURES",
                               contributor = NULL,
                               path = NULL,
  ...
  version_pattern <- sprintf("^#\\s+%s\\s+\\(%s\\)", 
                             basename(path), 
                             gsub(".", "\\\\.", version))
  ...
  if (length(version_idx) == 0) {
    ...
    if (length(non_empty_idx) == 0 || !open_section) {
      ...
      if (length(cat_idx) > 0 && cat_idx[1] > idx && 
          (length(next_version_idx) == 0 || cat_idx[1] < idx + next_version_idx[1])) {
        # Category exists, add entry there
        # Find where to insert (after category header and blank line)
        insert_pos <- cat_idx[1] + 1
        while (insert_pos <= section_end && !nzchar(trimws(lines[insert_pos]))) {
...
#' }
#' }
news_md_check <- function(path = NULL, strict = FALSE, verbose = FALSE) {
  path <- path %||% getwd()
  news_path <- file.path(path, "NEWS.md")
  ...
  if (verbose) {
    cli::cli_inform("Checking NEWS.md with {length(lines)} lines")
  }
  ...
  for (i in seq_along(lines)) {
    ...
    if (grepl("^#\\s+\\S+\\s+\\S+\\s+\\(", line)) {
      ...
      if (!grepl("^#\\s+\\S+\\s+[0-9]+\\.[0-9]+\\.[0-9]+[^a-zA-Z]*\\s+\\([0-9]{4}-[0-9]{2}-[0-9]{2}\\)", line)) {
        msg <- sprintf("Line %d: Version header should follow format '# pkgname X.Y.Z (YYYY-MM-DD)'", line_num)
        if (strict) {
          result$valid <- FALSE
      ...
      matches <- regmatches(line, regexpr("[0-9]+\\.[0-9]+\\.[0-9]+[^\\)]*", line))
      if (length(matches) > 0) {
      ...
      if (line_num > 1 && nzchar(trimws(prev_line))) {
        msg <- sprintf("Line %d: Blank line recommended before version header", line_num)
    ...
    if (grepl("^##\\s+", line)) {
      if (!in_version_section) {
        msg <- sprintf("Line %d: Category header found outside version section", line_num)
        result$valid <- FALSE
        result$errors <- c(result$errors, msg)
      }
      ...
      standard_categories <- c(
        "NEW FEATURES", "BUG FIXES", "MINOR IMPROVEMENTS",
        "DOCUMENTATION", "DEPRECATED", "DEFUNCT", "BREAKING CHANGES",
        "PERFORMANCE", "TESTING", "INTERNAL CHANGES"
      )
      
      if (!toupper(category) %in% toupper(standard_categories)) {
        msg <- sprintf("Line %d: Non-standard category '%s'. Consider using standard categories like: %s",
                       line_num, category, paste(standard_categories[1:4], collapse = ", "))
        result$suggestions <- c(result$suggestions, msg)
      }
      
      # Check blank line before category header
      if (nzchar(trimws(prev_line)) && !grepl("^#", prev_line)) {
        msg <- sprintf("Line %d: Blank line recommended before category header", line_num)
        result$suggestions <- c(result$suggestions, msg)
      }
    }
    ...
    if (grepl("^\\s*\\*\\s+", line)) {
      ...
      if (!grepl("^\\* [A-Z]", line)) {
        msg <- sprintf("Line %d: Bullet points should start with '* ' followed by capital letter", line_num)
        result$suggestions <- c(result$suggestions, msg)
      }
      ...
      if (nchar(trimmed) > 50 && !grepl("[.!?]\\s*$", trimmed)) {
        msg <- sprintf("Line %d: Longer entries should end with punctuation", line_num)
        result$suggestions <- c(result$suggestions, msg)
      }
      ...
      if (grepl("@[a-zA-Z][a-zA-Z0-9_-]*", line)) {
        if (!grepl("\\(\\s*@[a-zA-Z][a-zA-Z0-9_-]*\\s*\\)", line)) {
          msg <- sprintf("Line %d: Contributor mentions should be in parentheses: (@username)", line_num)
          result$suggestions <- c(result$suggestions, msg)
        }
      }
      ...
      if (grepl("#[0-9]+", line)) {
        if (!grepl("\\(#[0-9]+\\)", line)) {
          msg <- sprintf("Line %d: Issue/PR references should be in parentheses: (#123)", line_num)
          result$suggestions <- c(result$suggestions, msg)
        }
      }
    }
    ...
    if (grepl("^\\s*\\*", line) && grepl("[0-9]{4}-[0-9]{2}-[0-9]{2}", line)) {
      msg <- sprintf("Line %d: Dates should be in version headers, not individual entries", line_num)
      result$suggestions <- c(result$suggestions, msg)
    }
  ...
  if (!has_version_header) {
    result$valid <- FALSE
    result$errors <- c(result$errors, "No version headers found. Use format: # pkgname X.Y.Z (YYYY-MM-DD)")
  }
  ...
  if (length(trailing_ws) > 0) {
    msg <- sprintf("Lines with trailing whitespace: %s", paste(trailing_ws, collapse = ", "))
    result$suggestions <- c(result$suggestions, msg)
  }
  
  # Check file ends with newline
  if (length(lines) > 0 && !grepl("^\\s*$", lines[length(lines)])) {
    result$suggestions <- c(result$suggestions, "File should end with a blank line")
  }
  
  # Summary message
  if (verbose) {
    if (result$valid) {
      cli::cli_inform(c("v" = "NEWS.md passed all required checks"))
    } else {
      cli::cli_inform(c("x" = "NEWS.md has {length(result$errors)} error(s)"))
    }
    
    if (length(result$warnings) > 0) {
      cli::cli_inform(c("!" = "{length(result$warnings)} warning(s)"))
    }
    
    if (length(result$suggestions) > 0) {
      cli::cli_inform(c("i" = "{length(result$suggestions)} suggestion(s) for improvement"))
    }
  }
...
news_md_show <- function(path = NULL, version = NULL, max_versions = NULL) {
  ...
  if (!file.exists(news_path)) {
    cli::cli_abort("NEWS.md not found at {.path {news_path}}")
  }
  ...
  if (!is.null(version)) {
    if (version == "latest") {
      ...
      end_idx <- if (length(version_starts) > 1) version_starts[2] - 1 else length(lines)
    } else {
      ...
      for (i in seq_along(version_starts)) {
        if (grepl(version, lines[version_starts[i]])) {
          ...
          end_idx <- if (i < length(version_starts)) version_starts[i + 1] - 1 else length(lines)
          found <- TRUE
          break
        }
      }
      if (!found) {
        cli::cli_abort("Version '{version}' not found in NEWS.md")
      }
    }
